...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    ...
    val groups =  points.par.groupBy(x => findClosest(x, means))
    means.par.map( x => (x , groups.par.getOrElse(x, List()))).toMap
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    ...
    (newMeans zip  oldMeans).par.forall{ case (x,y) =>  x.squareDistance(y) <= eta}
  }
