...
class KMeansSuite extends FunSuite {
  ...
  test("teste ao ktretas") {
    val p1 = new Point(0, 0, 1)
    val p2 = new Point(0,0, -1)
    val p3 =  new Point(0,1,0)
    val p4 = new Point(0,10,0)
    val pointsA: GenSeq[Point] = IndexedSeq(p1, p2, p3, p4)
    val p5 =  new Point(0, -1, 0)
    val p6 =  new Point(0, 2, 0)
    val pointsB: GenSeq[Point] = IndexedSeq(p5, p6)

    assert(kMeans(pointsA, pointsB, 12.25).toVector != Vector((0.0, 0.0, 0.0), (0.0, 5.5, 0.0)))
  }
